...
# 불용어 정의
#stopwords = ['의','가','이','은','들','는','좀','잘','걍','과','도','를', '을', '으로','자','에','와','한','하다', '라', '로', '수', '고', '것', '게', '까지', '다', '처럼', '에서', '그', '지', '건', '면', '데', '정말', '이야', '그리고', '이고', '걸', '그', '채', '이란', '만큼', '이다', '에다', '저', '에는', '이라는', '그렇게', '인', '거야', '한테']
...
class text_segmentation_class:
    ...
    def tokenize_doc(self):
        ...
        all_doc_tokens = self.doc.split(" ")
        
        #all_doc_tokens = okt.morphs(self.doc)
        token_index = []
        doc_tokens = []
        
        for i, w in enumerate(all_doc_tokens):
            tmp = okt.morphs(w, stem = True)
            if len(tmp) > 0:
                x = tmp[0]
                doc_tokens.append(x)
    ...
    def get_word_embed_and_entropy(self):
        ...
        for word in voc_word_list:
            if not(word in self.model.vocab):
                ...
                self.voc_dict[word]['em'] = self.model[word]
